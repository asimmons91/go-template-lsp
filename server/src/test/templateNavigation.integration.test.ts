...
test('completes a template name', async () => {
  ...
  try {
    ...
    const list = await result.mode.doComplete(document, document.positionAt(offset), result.regions);
...
test('goes to definition from a template reference', async () => {
  ...
  try {
    ...
    const defs = (await result.mode.doDefinition!(document, document.positionAt(offset), result.regions)) ?? [];
    assert.equal(defs.length, 1);
    assert.ok(defs[0].uri.endsWith('layouts/base.gohtml'), `expected base.gohtml, got: ${defs[0].uri}`);
  } finally {
    languageModes.dispose();
  }
});
...
test('finds references to a define', async () => {
  ...
  try {
    ...
    const refs = (await result.mode.doReferences!(document, document.positionAt(offset), result.regions, { includeDeclaration: false })) ?? [];
    assert.equal(refs.length, 2);
    const uris = refs.map((r) => r.uri).sort();
    assert.ok(uris.every((u) => u.endsWith('views/index.gohtml') || u.endsWith('views/other.gohtml')), uris.join(', '));
  } finally {
    languageModes.dispose();
  }
});
...
test('includes the declaration when requested', async () => {
  ...
  try {
    const { document, offset } = makeDocument('layouts/base.gohtml', '{{define "header');
    const result = modeAt(languageModes, document, offset);
    const refs = (await result.mode.doReferences!(document, document.positionAt(offset), result.regions, { includeDeclaration: true })) ?? [];
...
test('block is both a definition and a reference', async () => {
  ...
  try {
    ...
    const defs = (await result.mode.doDefinition!(document, document.positionAt(offset), result.regions)) ?? [];
    assert.equal(defs.length, 1);
    assert.ok(defs[0].uri.endsWith('views/index.gohtml'), `expected index.gohtml, got: ${defs[0].uri}`);
  } finally {
    languageModes.dispose();
  }

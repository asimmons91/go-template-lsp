...
test('html tag completion outside any embedded region', async () => {
  ...
  assert.ok(
    labels.includes('div'),
    `expected 'div' among HTML tag completions, got: ${labels.join(', ')}`,
  );
});
...
test('css property completion inside <style>', async () => {
  ...
  assert.ok(
    ...
    `expected 'color' among CSS completions, got: ${labels.join(', ')}`,
  );
});
...
test('js member completion inside <script>', async () => {
  ...
  assert.ok(
    labels.includes('log'),
    `expected 'log' among JS completions, got: ${labels.join(', ')}`,
  );
});

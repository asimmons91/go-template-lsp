...
test('merges Go, CSS and JS diagnostics into one list', async () => {
  ...
  try {
    ...
    assert.ok(diags.some((d) => d.source === 'go-template' && /Unexpected \{\{end\}\}/.test(d.message)));
    assert.ok(diags.some((d) => d.source === 'css'), 'expected CSS diagnostics');
    assert.ok(diags.some((d) => d.source === 'typescript'), 'expected TS diagnostics');
  } finally {
...
test('reports no diagnostics on a clean document', async () => {
  const languageModes = getLanguageModes('gopls', undefined);
  try {
    const doc = documentFor('<style>.x { color: red; }</style><script>const x = 1;</script><p>hi</p>');
    const diags = await languageModes.doDiagnostics(doc);
